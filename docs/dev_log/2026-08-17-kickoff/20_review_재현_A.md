# R-재현 리뷰 — 트랙 A (데이터 파이프라인)

리뷰어 트랙 D · 2026-08-17 · 대상 `10_spec_A_데이터파이프라인.md` + `wt/A` 구현 (main `1af1d03`)
루브릭 `15_리뷰루브릭_R적대.md` · 원저자 배제 원칙에 따라 D가 수행

**요약: Critical 0 / Important 1 / Minor 2 — 머지 유지 의견(이미 머지됨). 조건부 후속 2건.**

A의 구현은 재현된다. 82건 전부 통과했고, 스냅샷 해시는 독립 계산으로 전건 일치했으며,
누수 불변식(묶음이 split·client를 가로지르지 않음)은 실제 데이터에서 위반 0건이었다.
Important 1건은 A의 결함이 아니라 **A와 D의 스펙이 서로 다른 기본값을 가정하고 있는
접점 문제**이고, 게이트 결정이 필요하다.

---

## 1. 재현 절차와 결과

전부 실제로 실행했다. 검증 스크립트는 이 리뷰 말미 §5에 경로를 적었다.
...
D가 소비자로서 가장 신경 쓰는 항목이다. 여기가 깨지면 5칸 채점이 통째로 무의미해진다.
...
2. `conditional`로 승격할지 여부는 **총괄 결정 사항**으로 올린다. 승격하려면 A가
   `assumptions` 블록을 채워야 하며, 그 값이 곧 논문 각주가 된다.
3. D는 `clause_only`를 **기본 경로로 가정하고** 채점기를 만든다. 즉 판정 정합성이 `N/A`인
   상태에서도 나머지 12개 지표가 전부 나오는지를 mock으로 먼저 확인한다. 이건 D가
   지금 바로 할 수 있고, A의 조치를 기다리지 않는다.

> D 스펙 수정이 처방의 절반이므로, 이 항목은 A에 대한 지적이라기보다 **접점 결정
> 요청**이다. 게이트에 올린다.
...
**처방.** 분할 직후 어서션 1줄. "eval에 각 `strata_key`가 최소 1건, 각 결함 클래스가 최소
K건(K는 configs)"을 검사하고 미달 시 경고와 함께 목록을 남긴다. 실패가 아니라 경고면
충분하다 — 희소 조합이 실제로 존재하는 데이터도 있을 수 있다. D 쪽에서도 `metrics/`가
...
리뷰어가 봤고 문제가 없었던 항목이다. 다음 리뷰어가 중복 조사하지 않도록 남긴다.
...
리뷰에 쓴 스크립트 2종을 저장소에 남긴다(리뷰 재현용, 실험 경로 아님).
...
**Critical 0 / Important 1 / Minor 2 — 머지 유지. 후속 2건(Important 1은 게이트 결정, Minor 2는 원저자 재량).**
