...
#:
#: - ``ABS_ORIG``    : 원본 이미지 절대 픽셀. 변환이 항등이다.
#:                     Ultralytics ``Results.boxes.xyxy`` 처럼 프레임워크가 이미 원본으로
#:                     복원해 주는 경로가 여기 해당한다(D의 Q20 실측 봉인 대상).
#: - ``NORM_1000``   : 원본 기준 0~1000 정규화. Qwen2-VL 계열 규약이며 우리 기본값이다.
...
class CoordCfg:
    ...
    이 설정값 하나로 흡수한다. 카나리아-1(파인튜닝 전 제로샷 grounding argmax 판별)이
    실측으로 규약을 판별하면 ``coord_space`` 값만 바꾸며, 모듈·채점기 코드는 불변이다.
